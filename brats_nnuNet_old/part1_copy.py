...
def generate_scorecam(predictor, model, case_files):

    cam_extractor = ScoreCAM(model, target_layer='decoder.seg_layers.4')
    ...
    # 🔥 IMPORTANT: Score-CAM needs forward pass ONLY (no gradients)
    with torch.no_grad():
        output = model(data)

    # use ET class
    activation_map = cam_extractor(class_idx=3, scores=output)[0]
...
if __name__ == "__main__":
    ...
    for patient_path in tqdm(patients):
        ...
        cam = generate_scorecam(predictor, model, case_files)
